...
- [修复] AI 选股 `/picker` 页补齐 V4 历史任务展示归一化：任务概览中的 `高级因子增强 / AI复核 / AI软否决` 计数会从候选数据推断补齐，历史遗留候选摘要若仍引用修复前分数或行动建议，也会在读取时按最终 `total_score + trade_plan.action` 自动重建，避免页面继续显示旧摘要。
- [测试] Web `vitest` 补齐 `ResizeObserver` 测试环境 mock，恢复 `/picker` 页面测试在新滚动同步逻辑下的稳定通过。
- [文档] 补充 AI 选股 V4 历史回放验证与页面级 E2E 验收记录：确认 `2026-03-20` 自选股历史样本在三个内置模板下已完成 `5/10` 日回放验证，并记录 `/picker` 页任务概览计数与候选摘要一致性复验结果；真实自然样本与 `000300` 真实任务 E2E 仍保持未关闭。
- [修复] AI 选股 `/picker` 页的“最近任务”和“候选列表”面板在宽屏双栏布局下改为与左侧卡片等高并在内部滚动，修复内容区高度未占满卡片、列表撑高整列的问题。
- [文档] 调整 AI 选股 `V5 / V6` 版本边界：`V5` 收敛为研究底座补强、模板版本化、多基准评估与 AI 研究副驾，原先暂缓的 `AI 选板块` 与工作流放大器能力统一顺延到 `V6`。
- [文档] 在 AI 选股技术设计文档中补充可直接拆分 issue 的 `V5 开发清单` 与 `V6 预留清单`，明确每阶段的建议任务粒度与实现边界。
- [文档] 进一步细化 AI 选股 `V5` 研发拆分：在技术设计文档中补充 `Epic -> Issue` 级别的实施建议、建议顺序与主要改动面，便于按当前代码结构直接排期落地。
...
- [修复] AI 选股 `/picker` 运行时构造 `market_regime_snapshot` 改为优先复用 `000300` 指数专用补数链路，不再先把基准当普通个股走通用数据源，修复 V4 任务因 `000300` 数据源连续失败而整任务提前中断的问题。
- [新功能] AI 选股进入 `V4.2 Phase 1`：新增 shortlist 级高级因子增强，围绕相对强弱、板块地位、流动性质量与事件强度对候选进行二次加分，并将结果写入 `advanced_factors`、评分拆解与最终排序链路。
- [改进] AI 选股 TopK 候选新增 AI 二次复核与首版软否决机制：复核结果会输出 supporting / counter / veto reasons，按 `pass/caution/soft_veto` 映射 `ai_review_penalty`，并在 `soft_veto` 时把 `trade_plan.action` 下调为 `observe`，但不硬移除候选。
- [改进] AI 选股 `/picker` 页与 API 补充 `advanced_factors / ai_review / template_failure_flags` 透出，任务摘要新增 `advanced_enriched_count / ai_reviewed_count / ai_soft_veto_count`，便于跟踪 V4.2 增强链路是否真实生效。
- [新功能] AI 选股进入 `V4.2 Phase 2`：新增 `/api/v1/picker/stats/validation` 与 `/api/v1/picker/stats/risk`，分别输出样本外验证、月滚动验证，以及 `profit_factor / return_drawdown_ratio / P25-P50-P75` 等风险调整后指标。
- [改进] AI 选股候选后验评估新增 `mfe_pct / mae_pct` 落库与前端展示，历史回放与回补脚本同步输出 Phase 2 所需的 excursion 指标。
- [改进] AI 选股补完 `V4` 开发收口：结构化解释补齐高级因子、研究/执行置信度与软否决的一致性表达，模板失效标记新增相对强弱转弱/板块逆风/流动性支撑不足识别，排序稳定性同步参考行动建议、失效等级与研究/执行置信度，减少同分候选漂移。
- [文档] 同步 AI 选股 PRD / 技术设计到 `V4.2 Phase 2` 代码完成状态，并继续保留真实自然样本后验、样本外稳定性与 `000300` 基准 E2E 为未关闭验收项。
- [文档] 同步 AI 选股 PRD / 设计文档到 `V4.2 Phase 1` 实现状态，并继续保留 `V4.1` 真实自然样本后验与 `000300` 基准 E2E 为未关闭验收项。
- [新功能] AI 选股进入 `V4.1 Phase 3`：新增置信度校准统计接口 `/api/v1/picker/stats/calibration`，按 `模板 × 市场环境 × 规则版本 × 信号桶` 输出样本数、名义概率、真实命中率、校准偏差及收益/超额/回撤表现，并在 `/picker` 页面增加最小可见校准面板。
- [改进] AI 选股 `research_confidence` 接入首版高置信度 gate：门槛绑定 `可比样本量 / 平均超额收益 / 校准偏差 / 高信号桶相对中低桶表现`，候选详情会展示当前分桶、校准偏差、口径绑定信息与未达标原因；未通过时仍保守降级为“中性 / 观察中 / 样本不足”。
- [文档] 记录 AI 选股当前阶段结论：`V4.1` 除真实自然样本相关验收外，代码项已补齐到 `Phase 3`，研发节奏上可进入 `V4.2`；但 `V4.1` 仍保留自然样本后验与 `000300` 基准 E2E 为未关闭验收项。
- [新功能] AI 选股进入 `V4.1 Phase 2`：新任务规则版本升级为 `v4_1_phase2`，A 股候选新增最小执行约束与成本模型，覆盖流动性分层、跳空风险、近似不可成交识别、滑点估计与 `execution_penalty`，并接入候选总分与 `trade_plan`。
- [改进] AI 选股 `/picker` 候选详情新增 `execution_constraints / research_confidence / execution_confidence` 展示；研究置信度在未完成校准前仅输出“样本不足 / 观察中 / 待校准”等保守状态，不展示“高置信度”。
- [改进] AI 选股市场环境与分层统计补齐稳态兜底：`000300` 市场环境快照在基准缓存不足时会复用指数补数链路，基础分层统计固定返回环境 / 模板 / 排名层 / 信号强弱空桶，避免页面因样本暂缺失而整组消失。
- [文档] 同步 AI 选股 PRD / 技术设计到 `V4.1 Phase 2` 代码完成状态，并继续保留真实自然样本后验、`000300` 基准 E2E、概率分桶校准与高置信度门槛为后续验收项。
- [新功能] AI 选股新增历史回放验证脚本 `scripts/replay_picker_history.py`：可按历史日期回放模板选股并直接计算 `5/10/20` 日后验，用于在真实样本尚未自然成熟前提前验证候选收益与回撤口径；默认禁用新闻/AI 并支持 `benchmark_mode=local_only` 避免基准缺失时长时间联网补数。
- [改进] AI 选股历史回放与后验评估补齐 `000300` 指数专用补数链路：在 `fetch_missing` 模式下优先走 `Tushare index_daily(000300.SH)`，兜底走 `YFinance 000300.SS`，并统一回写到本地 `stock_daily(code='000300')`，使历史回放可直接产出 `benchmark_return_pct` 与 `excess_return_pct`。
- [修复] AI 选股模板统计接口改为先按整数接收 `window_days` 再交由服务层校验，修复 `/picker` 页面默认统计窗口 `10` 在浏览器 query string 场景下被误判为非法并在首屏展示请求失败的问题。
- [修复] AI 选股 `/picker` 页候选列表增加高度上限与内部滚动，避免长列表持续撑高页面布局。
- [新功能] AI 选股 V3 第二阶段上线离线后验回补：新增 `scripts/backfill_picker_evaluations.py`，可按任务 / 日期批量补齐 `5/10/20` 日后验评估并为模板统计回填样本，模板统计接口同步改为纯读库，不再在读请求中触发全历史补算。
- [改进] AI 选股 `/picker` 页补充 V3 第二阶段板块质量增强：行业目录新增强势/中性/弱势、涨跌榜方向、排名与涨跌幅元信息，任务概览同步展示板块质量摘要，新的选股任务规则版本提升为 `v3_phase2`。
- [修复] AI 选股 `/picker` 页补齐运行中任务概览兜底：运行态任务即使 summary 尚未完整落库，也会展示任务级总股票数、已处理数、基准与筛选策略，不再出现整块全 `0` 或 `--`。
- [修复] 补齐 AI 选股 V3 第一阶段收尾：新建选股任务的规则版本改为显式记录 `v3_phase1`，避免 V1 旧版本号继续混淆当前口径。
- [改进] AI 选股 `/picker` 页补充 V3 口径可见性：任务概览新增规则版本、目标交易日、基准与筛选策略说明，模板统计补充可比样本与基准缺失计数，候选后验详情修正 `benchmark_unavailable` 提示。
...
- [文档] 收敛 AI 选股规划文档为 PRD 与技术设计两份主文档，补充 V1 已完成状态、V2 手动板块模式规划、V3 AI 选板块规划与可直接开工的 V2 开工清单。
...
- [新功能] AI 选股 V2 新增双模式运行：保留自选股模式，同时新增手动 A 股行业板块模式；运行接口支持 `mode`、`sector_ids`、`ai_top_k`、`notify`，Web `/picker` 页面新增板块选择、解释数量和完成后通知开关。
- [新功能] AI 选股 V2 新增后验评估闭环：为候选结果落库存储 `5/10/20` 日后验表现，任务详情返回收益率、超额收益、最大回撤等评估数据，并新增模板统计接口与页面面板。
- [修复] 补齐 AI 选股 V2 后验闭环：候选详情抽屉新增 `5/10/20` 日后验表现展示，任务详情读取时会自动补算已到期窗口，避免必须先访问模板统计接口才刷新评估结果。
- [修复] 修正 AI 选股 V2 模板胜率统计口径：当 `000300` 基准收益暂不可用时，不再把该样本计入胜率分母，避免模板表现被系统性低估。
- [修复] 修复 AI 选股 V2 板块模式任务启动即失败的问题：`StockPickerService._run_task()` 恢复注入运行时 `config` 后再初始化 `GeminiAnalyzer`，避免板块/自选股任务因 `name 'config' is not defined` 直接报错。
- [文档] 同步 AI 选股 PRD 与技术设计文档到 V2 已实现状态，明确首版板块模式、参数边界、统计窗口与 V3 `AI 选板块` 的延后规划。
- [文档] 补充 AI 选股 V3 之后的专业化扩展分析，新增数据层、策略层、板块研究层、评估层、组合层、执行层与 AI 研究副驾层的长期演进方向，并补充所需数据源与平台能力建议。
- [文档] 细化 AI 选股 V4 专业化口径：同步补充 PRD 与技术设计文档中的研究/执行双置信度定义、概率分桶校准、样本外与滚动验证、模板失效条件，以及“高置信度”标签的展示门槛。
- [文档] 明确 AI 选股 A 股阶段的数据源方案：补充公开免费源与免费但有限额/有限权源的纳入范围，并梳理 `.env.example` 已有配置入口与仍需额外集成的官方数据源。
- [文档] 重排 AI 选股后续路线：将 `V5` 调整为“A 股专业深化版”，保留 `AI 选板块` 并前移承接通往最终形态所需的研究能力与数据源建设；原先偏工作流放大器的能力顺延至 `V5` 之后。
- [文档] 明确 AI 选股 `V4.2` 与 `V5` 的边界：`V4.2` 只保留当前数据底座上的单票精度与置信度增强，板块研究、策略对象化、多基准研究框架与 AI 研究副驾等重叠部分统一收拢到 `V5`。
- [文档] 补充 AI 选股 `V4 / V5 / V5之后` 能力边界对照，并同步将自然语言策略输入从 `V5` 顺延到 `V5` 之后，避免版本定义前后矛盾。
- [改进] AI 选股 V2 将 `ai_top_k` 可配置上限从 `8` 放宽到 `10`，同步更新前后端校验与页面默认值，避免板块模式下解释数量设为 10 时触发 422。
- [改进] AI 选股任务详情改为纯读库返回，不再在打开历史任务时同步补算后验评估，避免详情页受 Tushare 等外部数据源时延影响。
- [文档] 同步 AI 选股 V2 验证进度：记录 2026-04-14 已完成的板块模式实测、`ai_top_k` 上限调整、历史任务详情纯读库改动与后验 `待更新` 的预期行为。
- [修复] 收口 AI 选股 V2 页面与 API 对齐：板块列表补充说明字段并在页面展示市场/覆盖数/说明，模板统计接口 `window_days` 公开约束收紧为 `5/10/20`，统计与后验基准文案改为读取接口返回的 `benchmark_code`。
- [文档] 明确 AI 选股 V2 剩余验证节奏：`5/10/20` 后验计算正确性、模板统计与 `000300` 基准稳定性需在观察窗口成熟后基于更早历史任务补验，暂不作为 2026-04-14 当天阻断项。
- [文档] 重排 AI 选股版本规划：将 V3 调整为“核心完善版”，优先收口数据口径、模板质量、解释一致性与后验补验；原先的 `AI 选板块`、定时自动选股、自然语言策略输入等闭环增强能力顺延至 V4。
- [文档] 调整 AI 选股后续路线：新增 V4“精度增强版”，聚焦交易规则、分层统计、市场环境过滤、有效因子与真实执行约束；原先的闭环增强能力顺延至 V5。
- [文档] 补充 AI 选股 V4 分阶段实施建议：明确 `V4.1` 先收口后验补验、交易规则、市场环境、执行约束与基础分层统计，`V4.2` 再推进专业因子、板块研究、组合联动与 AI 研究副驾。
- [文档] 补充 AI 选股数据源边界判断：明确现有信息源足以支撑 A 股 `V4.1`，但 `V4.2` 与多市场专业化仍需补强正式行业分类、结构化公告 / 事件、研报 / 盈利预期修订及多基准序列。
- [改进] AI 选股 V3 收口核心判定链路：候选排序改为稳定排序并显式区分 `strict_match` 与质量门槛补位，结构化解释改为直接锚定评分拆解与技术快照，模板统计与候选后验补充可比性标记，降低结果漂移与“为凑满 TopN 强行补位”的噪音。
- [改进] AI 选股 V3 完成第一阶段收尾：任务详情补充交易日口径、板块目录快照与基准策略元信息，候选筛除增加统一原因码统计，后验评估将基准缺失显式标记为 `benchmark_unavailable`，进一步降低结果解释与统计口径漂移。
- [新功能] AI 选股进入 `V4.1 Phase 1`：内置模板补齐 `alpha 假设 / 适用环境 / 失效环境 / 排除条件` 与统一交易规则，任务规则版本升级为 `v4_1_phase1`，候选详情新增结构化 `trade_plan`、环境适配与信号强弱标签。
- [新功能] AI 选股新增基础市场环境与分层统计：任务级新增最小 A 股 `market_regime_snapshot`，候选排序接入模板环境适配降权与“observe”提示，并新增 `/api/v1/picker/stats/stratified` 接口及 `/picker` 页面分层统计面板。
- [文档] 同步 AI 选股 PRD / 技术设计到 `V4.1 Phase 1` 已实现状态，并继续明确：真实自然样本下的后验与 `000300` 基准端到端验证仍待后续补做，当前不能替代最终验收。
