...
| 选股 | AI 选股（V1） | Web `/picker` 页支持基于 `STOCK_LIST` 使用内置模板（趋势突破 / 强势回踩 / 均衡筛选）发起异步选股任务，输出 Top 20 候选并由 AI 解读前 10 名 |
...
> Web `/picker` 页新增 AI 选股 V1：当前版本只使用 `STOCK_LIST` 作为股票池，先做结构化打分，再由 AI 解释前排候选；V1 仅支持内置模板，不支持自由输入策略。
